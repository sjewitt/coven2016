...
function buildTree(obj,spacer){
	try{
    if(!spacer){
  		spacer = "";
  	}
  	
  	for(var a=0;a<obj.childArray.length;a++){
  	    page = new Page(obj.childArray[a].pageId);
  	    out += spacer + "<li id=\"node_" + obj.childArray[a].id + "\" title=\"" + page.linkText + "\">";
  	    //out += page.linkText;
        if(obj.childArray[a].hasChildren){
    			out += "\n" + spacer + "<ul>\n"
    			buildTree(obj.childArray[a], spacer + "  ");
    			out += spacer + "</ul>\n";
    		}
      out += "</li>\n";
  	}
...
*/
//var nodeTree = new Node(inNode.id,true);  //load childs
//showObject(inNode);
//buildTree(nodeTree,"");
//out = "<ul>" + out + "</ul>"  
